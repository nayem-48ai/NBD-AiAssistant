...
const ChatView: React.FC<ChatViewProps> = ({ messages, onSendMessage, onEditMessage, isLoading, onMenuClick }) => {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-[#0b0f1a] text-slate-200 overflow-hidden">
          ...
          {messages.map((msg) => (
            <div key={msg.id} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
              <div className={`group relative max-w-[90%] md:max-w-[85%] ${msg.role === 'user' 
                ...
                {msg.image && (
                  <img src={msg.image} alt="User Attachment" className="max-w-full rounded-xl mb-4 border border-white/10 shadow-lg" />
                )}
      ...
      {/* Simple Minimalist Input Area - Fixed Alignment and Icons */}
      <div className="px-4 md:px-8 pb-8 pt-2">
        <div className="max-w-3xl mx-auto relative">
          {selectedImage && (
            <div className="absolute bottom-full mb-4 animate-message">
              <div className="relative group">
                <img src={selectedImage} alt="Upload Preview" className="h-24 w-24 object-cover rounded-2xl border border-slate-700 shadow-2xl ring-4 ring-indigo-500/5" />
          ...
          <div className="bg-slate-800/40 border border-slate-700/60 rounded-[1.8rem] flex items-center gap-1 px-3 py-2.5 focus-within:border-indigo-500/50 transition-all shadow-2xl backdrop-blur-sm overflow-hidden">
            <button
            ...
            <textarea
              ...
              className="flex-1 bg-transparent border-none focus:ring-0 text-[16px] text-white py-2.5 max-h-40 resize-none no-scrollbar placeholder-slate-500 leading-normal font-medium align-middle outline-none"
              onKeyDown={(e) => {
          ...
          <p className="text-[9px] text-center text-slate-600 mt-4 uppercase tracking-[0.4em] font-black pointer-events-none">Powered by Net BD Pro</p>
        </div>
      </div>
    </div>
  );
